...
async function fazerLogin() {
  ...
  try {
    ...
    persistRememberedLogin('tv', email, remember);
    iniciarTV();
  } catch(e) {
...
async function carregar() {
  ...
  carregando = true;
  try {
    const data = await apiCall('GET', '/api/tv/queue?limite=50');
    const preparando = (data.preparando || []).filter(p => ['pendente','confirmado','em_preparo'].includes(p.status));
    const pronto     = (data.pronto || []).filter(p => p.status === 'pronto' && visivelAposConclusao(p, 'tempo_pronto'));
    const entregues  = (data.entregues || []).filter(p => visivelAposConclusao(p, 'tempo_entrega'));
...
function visivelAposConclusao(p, campoConclusao) {
  const referencia = p[campoConclusao] || p.updated_at || p.created_at;
  if (!referencia) return true;
  const concluidoEm = new Date(referencia).getTime();
  if (Number.isNaN(concluidoEm)) return true;
  return Date.now() - concluidoEm < TEMPO_VISIVEL_POS_CONCLUSAO_MS;
}
